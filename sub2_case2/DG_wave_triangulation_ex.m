function [L2_error,H1_error, DG_broken_error,r_2,CPU_time]=DG_wave_triangulation_ex(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,T,constant,coefficient)
% 04/26/2016
...
for n=0:N_T-2
    ...
    b = b1;
    clear b1 
    %b=(delta_t)^2*b+2*M*r_1-M*r_0+(delta_t)*Ae*r_1-(delta_t)^2*Az*r_1;  
    %left_matrix=M+delta_t*Ae
    %b=(delta_t)^2*b+2*M*r_1-M*r_0+(delta_t)/2*Ae*r_0-(delta_t)^2/2*Az*r_0;
    %left_matrix=M+delta_t/2*Ae+(delta_t)^2/2*Az;
    %b=(delta_t)^2*b+2*M*r_1-M*r_0+(delta_t)*Ae*r_1-(delta_t)^2*Az*r_1;  
    %left_matrix=M+delta_t*Ae;
    %scheme4
    b=2*(delta_t)^2*b+(4*M-2*Az*(delta_t)^2)*r_1+(delta_t*Ae-2*M)*r_0;  
    left_matrix=2*M+delta_t*Ae;
    % Solve
    r_2=left_matrix\b;
    clear b
    CPUTime(n+1,1) = toc;
