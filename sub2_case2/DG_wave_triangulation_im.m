function [L2_error,H1_error, DG_broken_error,r_2,CPU_time]=DG_wave_triangulation_im(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,N_T,T,constant,coefficient)
% 04/26/2016
...
for n=0:N_T-2
    ...
    clear b1 
    %scheme3
    b=2*(delta_t)^2*b+4*M*r_1+(delta_t*Ae-2*M-Az*(delta_t)^2)*r_0;  
    left_matrix=2*M+delta_t*Ae+Az*(delta_t)^2;
    % Solve
    r_2=left_matrix\b;
    clear b
    CPUTime(n+1,1) = toc;
