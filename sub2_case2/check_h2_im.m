...
[L2_error_1,H1_error_1,DG_broken_error_1,data_1,CPU_time_1]=DG_wave_triangulation_im(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,N_T,T);

Iter_refinement=2;N_T=2^(Iter_refinement*2);
[L2_error_2,H1_error_2,DG_broken_error_2,data_2,CPU_time_2]=DG_wave_triangulation_im(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,N_T,T);

Iter_refinement=3;N_T=2^(Iter_refinement*2);
[L2_error_3,H1_error_3,DG_broken_error_3,data_3,CPU_time_3]=DG_wave_triangulation_im(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,N_T,T);

Iter_refinement=4;N_T=2^(Iter_refinement*2);
[L2_error_4,H1_error_4,DG_broken_error_4,data_4,CPU_time_4]=DG_wave_triangulation_im(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,N_T,T);
...
% ,N,N.^(-5/2),'k--');
h2=legend('$||u-u_h||_0$','$||u-u_h||_1$','$||u-u_h||_{\star}$','$ch^3$','$ch^2$','$ch^2$','Location','southwest'); %在图形的左下角创建图例 h2，用来标识图中的曲线
