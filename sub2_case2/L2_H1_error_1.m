function temp=L2_H1_error_1(DGM,DGT,X,func_name_1,t,basis_type,derivative_order_x,derivative_order_y,Gauss_coefficient_reference_triangle,Gauss_point_reference_triangle)
...
for n=1:size(DGT,2) %遍历每个三角单元
    ...
    for k=1:length(Gauss_coefficient_local_triangle)
        error=feval(func_name_1,Gauss_point_local_triangle(k,1),Gauss_point_local_triangle(k,2),t)...
             -fe_solution(Gauss_point_local_triangle(k,1),Gauss_point_local_triangle(k,2),uh_local,derivative_order_x,derivative_order_y,basis_type);
        temp=temp+Gauss_coefficient_local_triangle(k)*error^2;
    end   
end
temp=sqrt(temp);