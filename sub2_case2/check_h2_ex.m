...
[L2_error_1,H1_error_1,DG_broken_error_1,data_1,CPU_time_1]=DG_wave_triangulation_ex(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,T);

Iter_refinement=2;%N_T=2^(Iter_refinement*2);
[L2_error_2,H1_error_2,DG_broken_error_2,data_2,CPU_time_2]=DG_wave_triangulation_ex(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,T);

Iter_refinement=3;%N_T=2^(Iter_refinement*2);
[L2_error_3,H1_error_3,DG_broken_error_3,data_3,CPU_time_3]=DG_wave_triangulation_ex(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,T);

Iter_refinement=4;%N_T=2^(Iter_refinement*2);
[L2_error_4,H1_error_4,DG_broken_error_4,data_4,CPU_time_4]=DG_wave_triangulation_ex(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,T);

Iter_refinement=5;%N_T=2^(Iter_refinement*2);
[L2_error_5,H1_error_5,DG_broken_error_5,data_5,CPU_time_5]=DG_wave_triangulation_ex(Iter_refinement,basis_type,eipsilon,sigma,beta,theta,T);
...
disp(T2);
disp('p=2,N_T=2^(Iter_refinement*2)');
...
slopeH1=[1/2*eH1(1);1/8*eH1(1);1/32*eH1(1);1/128*eH1(1)];%1/512*eH1(1)];%1/2048*eDGbroken(1)];
...
semilogy(X,eL2,'b-s',X,eH1,'c-x',X,eDGbroken,'g-O',X,slopeL2,'k--',X,slopeH1,'y--',X,slopeDGbroken,'r--','LineWidth',1); %在图形窗口中绘制四条曲线，字符串参数分别指定数据样式和颜色（蓝色实线代表 eL2，绿色圆圈代表 eDGbroken，黑色虚线代表 slopeL2，红色虚线代表 slopeDGbroken)
% ,N,N.^(-5/2),'k--');
h2=legend('$||u-u_h||_0$','$||u-u_h||_1$','$||u-u_h||_{\star}$','$ch^3$','$ch^2$','$ch^2$','Location','northeast','Fontsize',4); %在图形的左下角创建图例 h2，用来标识图中的曲线
set(h2,'Interpreter','latex')  %设置图例中字符串的解释器为 LaTeX，以便支持 LaTeX 格式的文本
set(h2,'fontsize',15);  %设置图例中字符串的字体大小为15
set(gca,'XTick',1:L,'XLim',[0,L+1],'Ylim',[1e-6 1e2]);  %设置图形的横轴刻度，包含从1到 L 的整数
