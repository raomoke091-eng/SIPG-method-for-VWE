...
hold off;
h2=legend('$||u-u_h||_0$','$||u-u_h||_1$','$||u-u_h||_{*}$','$ch^3$','$ch^2$','$ch^2$','Location','southwest'); %在图形的左下角创建图例 h2，用来标识图中的曲线
set(h2,'Interpreter','latex')  %设置图例中字符串的解释器为 LaTeX，以便支持 LaTeX 格式的文本
set(h2,'fontsize',10);  %设置图例中字符串的字体大小为15
set(h2, 'Color', [1, 1, 1, 0.5]);  % 设置图例背景色为半透明

set(gca,'XTick',1:L,'XLim',[0,L+1],'Ylim',[1e-8 1e2]);  %设置图形的横轴刻度，包含从1到 L 的整数
