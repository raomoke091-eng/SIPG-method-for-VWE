...
%r=(1 - exp(t^2))*(1 - exp(y*(y - 1)))*((2*x - 1)^2 + 2)*exp(x*(x - 1));
...
%r=((2*x - 1)^2 + 2)*(exp(y*(y - 1)) - 1)*exp(-t + x*(x - 1));
%r=((2*x - 1)^2 + 2)*(exp(y*(y - 1)) - 1)*exp(x*(x - 1))*cosh(pi*t);
...
%r=((2*x - 1)^2 + 2)*exp(-2*t + x*(x - 1))*sin(pi*y);
r=((2*x - 1)^2 + 2)*(exp(y*(y - 1)) - 1)*exp(-2*t + x*(x - 1));