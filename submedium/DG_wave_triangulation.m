...
n_1 = 12; %12对应34,35,36
n_2 = 74; %74对应220,221,222
un_1 = zeros((basis_type+1)*(basis_type+2)/2, N_T-1);  %N_T-1次迭代
un_2 = zeros((basis_type+1)*(basis_type+2)/2, N_T-1);

un_cont = zeros(size(DGT, 2), (basis_type+1)*(basis_type+2)/2, N_T-1);  %%%行，列，层   三角形  每个三角形6个系数(p=2)  N_T-1个时刻
%接下来在每一次迭代中注意保存选定的4个点处每一次的数值解
for n=0:N_T-2
    tic
    t_0=n*delta_t; 
    t_1=(n+1)*delta_t; 
