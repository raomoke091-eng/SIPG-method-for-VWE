%生成积分区域为单元对应的单位刚度矩阵
function r=generate_stiffness_matrix_local_DG(func_term,DGM,DGT,Gauss_coefficient_reference_triangle,Gauss_point_reference_triangle,basis_type...
...
for n=1:size(DGT,2) %循环迭代处理每个三角形
    ...
    for k=1:length(Gauss_coefficient_local_triangle)
        a=triangular_local_basis(Gauss_point_local_triangle(k,1),Gauss_point_local_triangle(k,2),basis_type...
                                ,derivative_degree_x,derivative_degree_y); 
        %输入函数triangular_local_basis的点坐标(x,y)为当前遍历local高斯点坐标
        c=Gauss_coefficient_local_triangle(k)*feval(func_term,Gauss_point_local_triangle(k,1),Gauss_point_local_triangle(k,2),0);
        b=b+c*a'*a; %用就可以得到形函数两两相乘的结果：1*1,1*x,1*y,x*1,x*x,x*y,y*1,y*x,y*y
    end
    ...
    for i=1:(basis_type+1)*(basis_type+2)/2
        for j=1:(basis_type+1)*(basis_type+2)/2
            r((n-1)*(basis_type+1)*(basis_type+2)/2+i,(n-1)*(basis_type+1)*(basis_type+2)/2+j)...
                =r((n-1)*(basis_type+1)*(basis_type+2)/2+i,(n-1)*(basis_type+1)*(basis_type+2)/2+j)+b(i,j);
        end
    end
end
