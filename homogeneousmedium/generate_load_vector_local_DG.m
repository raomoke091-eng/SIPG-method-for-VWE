function r=generate_load_vector_local_DG(func_right_term,t,DGM,DGT,Gauss_coefficient_reference_triangle,Gauss_point_reference_triangle,basis_type)
...
for n=1:size(DGT,2)
    ...
    for k=1:length(Gauss_coefficient_local_triangle)
        for i=1:(basis_type+1)*(basis_type+2)/2
            ...
            b(i,1)=b(i,1)+Gauss_coefficient_local_triangle(k)*a(:,i)*feval(func_right_term,Gauss_point_local_triangle(k,1),Gauss_point_local_triangle(k,2),t);
        end
    end
